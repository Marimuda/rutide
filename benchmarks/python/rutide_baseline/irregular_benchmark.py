"""Pinned-Python throughput probe for irregular colored confidence intervals."""
...
def _solve_series(series: int) -> float:
    if _WORKER_STATE is None:
        raise RuntimeError("irregular benchmark worker was not initialized")
    state = _WORKER_STATE
    ...
    if state["field"] == "scalar":
        ...
        return float(coefficient.A_ci[0])
    coefficient = state["oracle"].solve(
        state["times"],
        state["eastward"],
        state["northward"],
    ...
    return float(coefficient.Lsmaj_ci[0])
